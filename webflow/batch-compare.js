(function () {
  ...
  async function initBatchCompare() {
    ...
    function splitDeepTextIntoSectionArray(text) {
      if (!text || typeof text !== "string") return [];
      var t = text.trim();
      var parts = t.split(/\n(?=\d+[.:])/);
      var out = [];
      for (var i = 0; i < 10; i++) out.push((parts[i] || "").trim());
      return out;
    }
    ...
    function appendDeepListening(card, item, token, opts) {
      ...
      if (unlocked && (item.openai_deep_text || "").trim()) {
        ...
        var sectionTexts = splitDeepTextIntoSectionArray(rawDeep);
        for (var s = 0; s < 10; s++) {
          var secDiv = document.createElement("div");
          secDiv.className = "sp-da-section";
          secDiv.setAttribute("data-sec", String(s + 1));
          secDiv.style.whiteSpace = "pre-wrap";
          secDiv.style.fontSize = "12px";
          secDiv.style.color = "#374151";
          secDiv.textContent = sectionTexts[s] || "";
        ...
        if (hasSuno) {
          ...
          toggleBtn.className = "sp-suno-toggle";
          var toggleLeft = document.createElement("span");
          toggleLeft.className = "sp-suno-toggle-left";
          var toggleTitle = document.createElement("span");
          toggleTitle.className = "sp-suno-toggle-title";
          toggleTitle.textContent = "Refine in Suno (Cover Plan)";
          var toggleSub = document.createElement("span");
          toggleSub.className = "sp-suno-toggle-sub";
          toggleSub.textContent = "2 surgical edits + copy/paste prompt";
          toggleLeft.appendChild(toggleTitle);
          toggleLeft.appendChild(toggleSub);
          var chevron = document.createElement("span");
          chevron.className = "sp-chevron";
          chevron.textContent = "▸";
          toggleBtn.appendChild(toggleLeft);
          ...
          promptWrap.className = "sp-suno-prompt-wrap";
          var promptHeader = document.createElement("div");
          promptHeader.className = "sp-suno-prompt-header";
          var promptTitle = document.createElement("span");
          promptTitle.className = "sp-suno-prompt-title";
          ...
          copyPromptBtn.className = "sp-suno-copy-btn";
          copyPromptBtn.textContent = "Copy";
          var promptTextarea = document.createElement("textarea");
          promptTextarea.className = "sp-suno-prompt";
          promptTextarea.readOnly = true;
          promptTextarea.value = (promptText && String(promptText).trim()) ? String(promptText).trim() : "";
          promptHeader.appendChild(promptTitle);
          promptHeader.appendChild(copyPromptBtn);
          promptWrap.appendChild(promptHeader);
          promptWrap.appendChild(promptTextarea);
          sunoContent.appendChild(promptWrap);

          copyPromptBtn.addEventListener("click", function () {
            var val = promptTextarea.value;
            function done() {
          ...
          toggleBtn.addEventListener("click", function () {
            var open = sunoContent.style.display !== "none";
            sunoContent.style.display = open ? "none" : "block";
            toggleBtn.classList.toggle("is-open", !open);
            toggleBtn.setAttribute("aria-expanded", !open ? "true" : "false");
          });

          sunoWrapper.appendChild(toggleBtn);
          sunoWrapper.appendChild(sunoContent);

          var sec9 = body.querySelector('.sp-da-section[data-sec="9"]');
          if (sec9 && sec9.insertAdjacentElement) {
            sec9.insertAdjacentElement("afterend", sunoWrapper);
          } else {
            body.appendChild(sunoWrapper);
          }
          } catch (e) {
            console.error("[Batch Compare] Suno CTA build failed", e);
          }
        }
      } else if (!unlocked) {
